...
def get_text_html(text_url):
	try:
		text_request = urllib.request.Request(text_url,None,headers)  
		text_response = urllib.request.urlopen(text_request,None)
		try:
	...
	fileHandle = open ( book_name+'.txt', 'a' )
...
try:
	...
	response = urllib.request.urlopen(request,None,timeout=30)
	try:
...
results = pool.map(get_text_html, text_urls)
pool.close()
pool.join()
end_time = time.time()
